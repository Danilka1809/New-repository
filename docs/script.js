// ========== КОНФИГУРАЦИЯ ==========
const CONFIG = {
    // Настройки почты (через Formspree)
    emailWebhook: 'https://formspree.io/f/твой_код_формы',
...
async function updateServerStatus() {
    try {
        ...
        if (badge) {
            ...
            if (statusText) {
                statusText.innerText = data.online ? 'Сервер онлайн' : 'Сервер офлайн';
            }
        }
    } catch (err) {
...
async function sendEmailNotification(nickname, privilegeName, price, server) {
    const subject = `🛒 Покупка доната на GrifLauncher`;
    const body = `
        Новая покупка!
    ...
    const response = await fetch('https://formspree.io/f/mrbppqad', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            name: nickname,
            email: CONFIG.emailTo,
            message: `Покупка: ${privilegeName} на сервере ${server}`,
            _subject: subject,
            _body: body
        })
    });
